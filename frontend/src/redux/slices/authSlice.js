...
export const addSavedDestination = createAsyncThunk('auth/addSavedDestination', async (destination, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    const config = { headers: { Authorization: `Bearer ${token}` } };
...
export const removeSavedDestination = createAsyncThunk('auth/removeSavedDestination', async (destination, thunkAPI) => {
  try {
    const token = thunkAPI.getState().auth.user.token;
    const config = { headers: { Authorization: `Bearer ${token}` } };
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(addSavedDestination.fulfilled, (state, action) => {
        if (state.profile) {
          state.profile.savedDestinations = action.payload;
        }
      })
      .addCase(removeSavedDestination.fulfilled, (state, action) => {
        if (state.profile) {
          state.profile.savedDestinations = action.payload;
        }
      });
